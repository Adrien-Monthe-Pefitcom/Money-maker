...
function SignUp(){
    ...
    // num compte marchant
    let nummarchantsupp = Math.floor(Math.random() *(90100000100 - 10100000100) + 90100000100);
    // $('#numcompte_comm').val(numcomm);

    info["num_marchant"] = nummarchantcommsupp;
...
function login(info) {
    $.ajax({
        ...
        url: '{{route('process-marchant-by-superviseur')}}',
        ...
        success:function(response){
            ...
            if(response.status=='success'){
                ...
                setTimeout(()=>{
                    window.location.href="{{route('liste-comptes-sup')}}";
                  }, 1000);
            }
