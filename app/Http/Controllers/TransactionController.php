...
class TransactionController extends Controller
{
    ...
    public function crediter_compte(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            } else if ($auth_account->solde_compte < $data['amount_trans']) {
                ...
                try {
                    ...
                    if(Auth::user()->role === "commercial"){
                        $updateusers = Commercial::where('id', $auth_account->id)->update([
                            'solde_compte' => $auth_account->solde_compte - $data['amount_trans']
                        ]);
                    }
                    elseif(Auth::user()->role === "superviseur"){
                    ...
                    return redirect()
                        ->route("credit")->withSuccess("Transaction Effectué avec Succès");
                } catch (SQLException $e) {
                    return redirect()
